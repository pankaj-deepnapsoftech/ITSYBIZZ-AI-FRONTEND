...
const filteredContact = contactList.filter((user) => {
  ...
  const handleView = (contactData) => {
    setSelectedContact(contactData); // Set selected contact data
...
<div className="w-full flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-5">
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-600 ">
       <thead className="text-xs  uppercase bg-gray-100 text-gray-800 bg-gradient-to-b from-gray-100 to-gray-100  rounded-lg   font-bold px-8 py-4  drop-shadow-md tracking-wide ">
          <tr>
      ...
      {isModalOpen && selectedContact && (
        <Modal onClose={() => setIsModalOpen(false)}>
          <DetailsComponent
            details={[
              { label: "Name", value: selectedContact.name || "Not Available" },
              { label: "Email", value: selectedContact.email || "Not Available" },
              { label: "Phone", value: selectedContact.phone || "Not Available" },
              { label: "City", value: selectedContact.city || "Not Available" },
              { 
                label: "Date", 
                value: selectedContact.date 
                  ? new Date(selectedContact.date).toLocaleDateString("en-GB", {
                      day: "2-digit",
                      month: "2-digit", 
                      year: "numeric",
                    })
